...
class Gamma_Loss(torch.nn.Module):
    def __init__(self):
    ...
    def forward(self, img1, img2):
        ...
        D1 = abs((mu1 + 2*sigma1)-(mu1-2*sigma1))
        D2 = abs((mu2 + 2*sigma2)-(mu2-2*sigma2))

        gamma1 = gammagen(mu1,sigma1,D1)
        gamma2 = gammagen(mu2,sigma2,D2)

        return abs(gamma1 - gamma2)

def gammagen(mu, sigma, D):
    # for low contrast image
    if D <= 1/3:
        gamma = -torch.log2(sigma)      
    # for high contrast or moderate contrast image
    else:
        gamma = torch.exp((1-(mu + sigma))/2)
